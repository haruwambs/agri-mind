...
async function checkSession() {
    ...
    if (session && session.user) {
        const { data: profile } = await db.from('profiles').select('display_name,email,phone,location,bio,created_at').eq('id', session.user.id).single();
