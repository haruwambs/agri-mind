...
async function loadJobs() {
    ...
    for (const j of jobs) {
        ...
        if (j.user_id) {
            ...
            else if (pf?.email) posterName = cleanDisplayName(pf.email);
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelector('.main-content').addEventListener('click', async (e) => {
        ...
        // FIXED: Contact button handler with validation
        if (target.closest('.contact-applicant-btn') || target.closest('.contact-poster-btn')) {
            ...
            if (email && email !== 'N/A' && email !== '') {
                ...
                showPage('messages');
                showToast(`Messaging ${name}`);
            } else {
                showToast('No email address available', true);
            }
        }
    });
