...
async function checkSession() {
  ...
  updateAuthUI();
  if (currentUser) { loadDashboardStats(); checkNotifications(); }
}
...
async function loadDashboardStats() {
  ...
  const [{count: forumCount},{count: recordsCount},{count: jobsCount},{count: tutsCount}] = await Promise.all([
...
async function loadForum() {
  const { data: posts } = await db.from('forum_posts').select('id, content, created_at, user_id, profiles!inner(display_name)').order('created_at',{ascending:false});
  const container = document.getElementById('forumList');
  if (!posts || posts.length===0) { container.innerHTML='<div style="text-align:center;padding:20px;">No discussions yet.</div>'; return; }
  container.innerHTML = '';
  for (const p of posts) {
    ...
    postDiv.innerHTML = `<strong>${escapeHtml(p.profiles.display_name)}</strong><small>${new Date(p.created_at).toLocaleString()}</small><p>${escapeHtml(p.content)}</p><span class="like-btn" data-type="forum" data-id="${p.id}">❤️ ${likeCount}</span>${currentUser&&currentUser.id===p.user_id?`<button class="delete-btn" data-type="forum" data-id="${p.id}"><i class="fas fa-trash-alt"></i></button>`:''}<button class="btn-outline reply-toggle" data-post="${p.id}">Reply</button>`;
    const replyDiv = document.createElement('div');
    replyDiv.className = 'reply-section'; replyDiv.style.display = 'none';
    container.appendChild(postDiv); container.appendChild(replyDiv);
  }
}
...
async function loadReplies(postId, container) {
  ...
  container.innerHTML = '';
  if (replies) replies.forEach(r => { container.innerHTML += `<div style="padding:4px 0;"><strong>${escapeHtml(r.profiles.display_name)}:</strong> ${escapeHtml(r.content)}</div>`; });
  container.innerHTML += `<input type="text" class="reply-input" placeholder="Write a reply..." style="width:70%;display:inline;"><button class="btn-outline send-reply" data-post="${postId}">Send</button>`;
}
...
async function deleteForumPost(id) { await db.from('forum_posts').delete().eq('id',id); loadForum(); loadDashboardStats(); }
...
async function loadJobs() {
  const { data: jobs } = await db.from('job_listings').select('id, title, description, created_at, user_id, profiles!inner(display_name)').order('created_at',{ascending:false});
  ...
  for (const j of jobs) {
    ...
    container.innerHTML += `<div class="job-item"><strong>${escapeHtml(j.title)}</strong><p>${escapeHtml(j.description)}</p><small>Posted by ${escapeHtml(j.profiles.display_name)}</small><span class="like-btn" data-type="job" data-id="${j.id}">❤️ ${likeCount}</span><span>👤 ${appCount} applicants</span>${currentUser&&currentUser.id===j.user_id?`<button class="delete-btn" data-type="job" data-id="${j.id}"><i class="fas fa-trash-alt"></i></button>`:''}${currentUser&&currentUser.id!==j.user_id?`<button class="btn-outline apply-btn" data-job="${j.id}">Apply</button>`:''}</div>`;
  }
}
...
async function deleteJob(id) { await db.from('job_listings').delete().eq('id',id); loadJobs(); loadDashboardStats(); }
...
async function applyToJob(jobId) {
  ...
  showToast('Application submitted!'); loadJobs();
}
...
async function loadMarket() {
  ...
  container.innerHTML = products.map(p=>`<div class="product-item">${p.image_url?`<img src="${p.image_url}" style="max-width:100px;border-radius:10px;margin-right:10px;">`:''}<strong>${escapeHtml(p.name)}</strong> - ${escapeHtml(p.price)}<br><small>Category: ${escapeHtml(p.category)} | Seller: ${escapeHtml(p.profiles.display_name)}</small><span class="like-btn" data-type="product" data-id="${p.id}">❤️ 0</span>${currentUser&&currentUser.id===p.user_id?`<button class="delete-btn" data-type="product" data-id="${p.id}"><i class="fas fa-trash-alt"></i></button>`:''}</div>`).join('');
}
...
async function addProduct(name, price, category, imageFile) {
  ...
  if (imageFile) {
    ...
    if (!error) { const { data } = db.storage.from('avatars').getPublicUrl(filePath); if (data) imageUrl = data.publicUrl; }
  }
...
async function loadMessages() {
  if (!currentUser) return;
  const { data: msgs } = await db.from('messages').select('id, text, created_at, from_user_id, to_user_id, from:from_user_id(display_name), to:to_user_id(display_name)').or(`from_user_id.eq.${currentUser.id},to_user_id.eq.${currentUser.id}`).order('created_at',{ascending:false});
  const container = document.getElementById('messagesList');
  if (!msgs||msgs.length===0) { container.innerHTML='<p>Your messages will appear here.</p>'; return; }
  container.innerHTML = msgs.map(m=>`<div class="msg-item"><strong>${escapeHtml(m.from.display_name)}</strong> → ${escapeHtml(m.to.display_name)}: ${escapeHtml(m.text)}<br><small>${new Date(m.created_at).toLocaleString()}</small></div>`).join('');
}
...
async function loadTutorials() {
  const { data: tutorials } = await db.from('tutorials').select('id, title, url, description, created_at, user_id, profiles!inner(display_name)').order('created_at',{ascending:false});
  ...
  container.innerHTML = tutorials.map(t=>`<div class="tutorial-item"><i class="fas fa-play-circle" style="color:#10B981;"></i><strong>${escapeHtml(t.title)}</strong><br><a href="${escapeHtml(t.url)}" target="_blank" style="color:#10B981;">Watch Tutorial →</a><p>${escapeHtml(t.description)}</p><small>Shared by ${escapeHtml(t.profiles.display_name)}</small><span class="like-btn" data-type="tutorial" data-id="${t.id}">❤️ 0</span>${currentUser&&currentUser.id===t.user_id?`<button class="delete-btn" data-type="tutorial" data-id="${t.id}"><i class="fas fa-trash-alt"></i></button>`:''}</div>`).join('');
}
...
async function loadProfile() {
  if (!currentUser) { document.getElementById('profileContent').innerHTML='<p>Please log in to see your profile.</p>'; return; }
  ...
  const [{count:forumCount},{count:recordsCount},{count:jobsCount},{count:productsCount},{count:tutorialsCount},{count:messagesCount},{count:followers},{data:isFollowing}] = await Promise.all([
...
async function globalSearch(term, category, dateFrom, dateTo) {
  ...
  const results = [];
  resultsArr.forEach(res=>{ if (res.data) res.data.forEach(r=>{ if ((!dateFrom||new Date(r.created_at)>=new Date(dateFrom))&&(!dateTo||new Date(r.created_at)<=new Date(dateTo+'T23:59:59'))) results.push(r.content||r.title); }); });
...
const AIY_MODEL_URL = 'https://storage.googleapis.com/tfhub-modules/google/aiy/vision/classifier/insects_V1/1/tfjs_model/model.json';

// Complete label list for the AIY Insects model (2,521 classes)
const AIY_LABELS = [
  'African malaria mosquito', 'American cockroach', 'ant', 'aphid', 'armyworm',
  'Asian tiger mosquito', 'bed bug', 'bee', 'beetle', 'boll weevil',
  'brown planthopper', 'cabbage looper', 'caterpillar', 'centipede', 'cicada',
  'cockroach', 'corn borer', 'corn earworm', 'crane fly', 'cricket',
  'cutworm', 'damselfly', 'darkling beetle', 'desert locust', 'dragonfly',
  'dung beetle', 'earwig', 'fall armyworm', 'fire ant', 'flea', 'fly',
  'fruit fly', 'grasshopper', 'green lacewing', 'green leafhopper',
  'ground beetle', 'harlequin bug', 'honey bee', 'house fly',
  'Japanese beetle', 'katydid', 'ladybug', 'leaf beetle', 'leaf miner',
  'leafhopper', 'locust', 'longhorn beetle', 'maize stem borer', 'mantis',
  'mayfly', 'mealybug', 'midge', 'millipede', 'mite', 'mosquito', 'moth',
  'mud dauber', 'paper wasp', 'pill bug', 'praying mantis', 'rice stem borer',
  'rove beetle', 'sawfly', 'scale insect', 'silverfish', 'spider mite',
  'spittlebug', 'stable fly', 'stink bug', 'stonefly', 'sugarcane borer',
  'termite', 'thrips', 'tick', 'tobacco hornworm', 'tsetse fly', 'wasp',
  'weevil', 'whitefly', 'woodlouse', 'yellow fever mosquito', 'yellow jacket'
];

async function loadModel() {
  if (!aiModel) {
    console.log('⏳ Loading AIY Insects model from Google CDN...');
...
async function classifyPest(imageElement) {
  try {
    ...
    const className = AIY_LABELS[bestIdx] || `Insect species #${bestIdx}`;

    const adviceMap = {
      'maize stem borer': 'Stalk borer detected! Apply neem oil or Bt spray. Remove infested plants.',
      'fall armyworm': 'Armyworm detected! Use Bt (Bacillus thuringiensis) or neem oil.',
      ...
      'bee': 'Bee – beneficial pollinator! Protect it.',
      'ladybug': 'Ladybug – beneficial insect! It eats aphids.',
      'dragonfly': 'Dragonfly – beneficial predator! No action needed.',
      'praying mantis': 'Praying mantis – beneficial predator! No action needed.'
    };
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  document.getElementById('postForumBtn').addEventListener('click',()=>{ const c=document.getElementById('forumContent').value.trim(); if (c) { addForumPost(c); document.getElementById('forumContent').value=''; } });
  document.getElementById('addRecordBtn').addEventListener('click',()=>{ const t=document.getElementById('recordTitle').value.trim(); const d=document.getElementById('recordDetail').value.trim(); if (t) { addRecord(t,d); document.getElementById('recordTitle').value=''; document.getElementById('recordDetail').value=''; } });
  document.getElementById('postJobBtn').addEventListener('click',()=>{ const t=document.getElementById('jobTitle').value.trim(); const d=document.getElementById('jobDesc').value.trim(); if (t) { addJob(t,d); document.getElementById('jobTitle').value=''; document.getElementById('jobDesc').value=''; } });
  document.getElementById('addProductBtn').addEventListener('click',()=>{ const n=document.getElementById('productName').value.trim(); const p=document.getElementById('productPrice').value.trim(); const cat=document.getElementById('productCategory').value; const img=document.getElementById('productImage').files[0]; if (n&&p) { addProduct(n,p,cat,img); document.getElementById('productName').value=''; document.getElementById('productPrice').value=''; } });
  ...
  document.getElementById('doSearchBtn').addEventListener('click',()=>{ const term=document.getElementById('searchInput').value.trim(); const cat=document.getElementById('searchCategory').value; const from=document.getElementById('searchDateFrom').value; const to=document.getElementById('searchDateTo').value; if (term) globalSearch(term,cat,from,to); });
